...
def get_bindings_and_params(args):
    ...
    if args.pool:
        if args.rs:
            pool = args.pool[np.random.randint(len(args.pool))]
        else:
            pool = args.pool[0]
        gin_bindings += ['POOL = ' + "'" + str(pool) + "'"]
        log_dir = log_dir + '_pool_' + str(pool)
    
    if args.frac:
        log_dir = log_dir + '_frac_' + str(args.frac)
        gin_bindings += ['DATA_FRAC = ' + str(args.frac)]

    if args.rtdl_emb:
        gin_bindings += ['RTDL_EMB = ' + str(args.rtdl_emb)]
    
    if args.path_to_ckpt:
        gin_bindings += ['PATH_TO_CKPT = ' + "'" + str(args.path_to_ckpt) + "'"]

    gin_bindings += ['PROJ = ' + str(args.proj)]
    if args.proj:
        log_dir = log_dir + '_proj'

    gin_bindings += ['FREEZE = ' + str(args.freeze)]
    if args.freeze:
        log_dir = log_dir + '_freeze'

    if args.add_cls:
        log_dir = log_dir + '_cls'
        gin_bindings += ['ADD_CLS = True']

    if args.cls_only:
        log_dir = log_dir + '_only'
        gin_bindings += ['CLS_ONLY = True']
